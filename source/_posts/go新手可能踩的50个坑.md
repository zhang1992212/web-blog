...
如果声明变量后未使用，则无法通过编译
#### 3. 未使用的import
如果import一个包，但包中的变量、函数、接口、结构体都未使用，则无法通过编译
#### 4. 简短声明的变量只能在函数内部使用
```go
    //错误示例
    a := 1
...
允许对值为nil的slice添加元素，但对值为nil的map添加元素则会造成运行时panic
```go
// map 错误示例
func main() {
...
```
#### 10. map容量
在创建map类型的变量是可以指定容量，但不能像slice一样使用cap()来检测分配空间的大小
```go
//错误示例
func main(){
    m := make(map[string]int, 99)
    fmt.Println(cap(m)) //error: invalid argument m1 (type map[string]int) for cap  
}
```
#### 11. string类型的变量值不能为nil
```go
// 错误示例
func main() {
...
#### 15. 检查map中key是否存在
```go
func MapKeyExist(key  string,checkMap map[string]interface{}) bool {
...
fmt.Println(MapKeyExist("c", a)) //false
fmt.Println(MapKeyExist("a", a)) //true
```
#### 16. string类型的值是常量，不可修改
```go
// 修改字符串的错误示例
func main() {
    x := "text"
...
![a](https://segmentfault.com/img/remote/1460000013739005?w=267&h=237)
...
```
#### 31. 程序退出是还有goroutine在运行
程序默认不等所有的goroutine都执行完成才退出，这个需要特别注意：
```go
// 主程序会直接退出
func main() {
    workerCount := 2
...
![](https://segmentfault.com/img/remote/1460000013739006?w=269&h=124)
...
```
执行结果：
![](https://segmentfault.com/img/remote/1460000013739007?w=464&h=373)
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739008?w=228&h=212)
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739009?w=214&h=77)
...
```
运行结果：
![](https://segmentfault.com/img/remote/1460000013739010?w=412&h=237)
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739011?w=230&h=147)
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739012)
...
```
运行结果：
![](https://segmentfault.com/img/remote/1460000013739013?w=425&h=129)
### 中级篇：35-50
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739014?w=480&h=134)
...
[](https://segmentfault.com/img/remote/1460000013739015?w=430&h=190)
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739016?w=209&h=209)
...
[](https://segmentfault.com/img/remote/1460000013739017?w=382&h=282)
...
```
运行效果：
![](https://segmentfault.com/img/remote/1460000013739018)
